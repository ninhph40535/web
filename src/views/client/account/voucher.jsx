...
const Vouchers = () => {
    ...
    console.log("User sau khi đăng nhập Vouvcher:", user);
    ...
    const GetAllByCaNhan = async () => {
        if (!customerId) {
            console.warn('Customer ID is null, skipping voucher fetch.');
            return; // Không gọi API nếu customerId là null
        }
        
        try {
            const response = await getAllByCaNhan(customerId);
            ...
            message.error('Đã xảy ra lỗi khi tải voucher cá nhân.');
        }
    };
    // Trigger API call on component mount
    useEffect(() => {
        GetAllByPublic(); // Gọi hàm này nếu cần
        GetAllByCaNhan(); // Chỉ gọi nếu customerId không phải là null
