...
//     request.get(`/payment/${bill.id}`).then((response) => {
...
//       key: 'method',
//       render: (x, record) => (
//         <Tag color={x === true ? 'green' : 'red'} style={{ width: "100px" }} className="text-center">{x === true ? 'Tiền mặt' : 'Chuyển khoản'}</Tag>
//       )
//     },
//     // {
//     //   title: 'Ghi chú',
...
function PaymentMethod({ bill, onSucess }) {
  ...
  const [tienKhachDua, setTienKhachDua] = useState(null);
  const [listBillDetail, setListBillDetail] = useState(null);
            ...
            {totalPayment < (bill.totalMoney + bill?.moneyShip) && (
              <>
                ...
                <PaymentModal
                  ...
                  setTienKhachDua={setTienKhachDua}
                  totalMoney={bill.totalMoney}
                  products={listBillDetail}
